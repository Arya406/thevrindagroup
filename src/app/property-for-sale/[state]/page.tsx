// ==============================================================================
// TheVrindaGroup - State Residential Sale Landing Route (/property-for-sale/[state])
...
interface StateSalePageProps {
  params: Promise<{ state: string }>;
}

export async function generateMetadata({
  params,
}: StateSalePageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://thevrindagroup.com/property-for-sale/${resolved.stateSlug}`;
  ...
  const title = `Properties for Sale in ${state.name} | TheVrindaGroup`;
  const description = `Explore verified properties for sale in ${state.name}. Browse available apartments, independent houses, villas, and plots listed directly on TheVrindaGroup.`;
...
export default async function StateSalePage({ params }: StateSalePageProps) {
  const { state: stateSlug } = await params;
  const resolved = resolveLocationParams(stateSlug);
  ...
  try {
    ...
    // Collect districts with active published listings in this state
